...
from reid.utils import AverageMeter, visualize_ranked_results, save_checkpoint, re_ranking, mkdir_if_missing
...
class Engine(object):
    ...
    Args:
        datamanager (DataManager): an instance of ``reid.data.ImageDataManager``
            or ``reid.data.VideoDataManager``.
    ...
    def __init__(self, datamanager, model, optimizers=None, schedulers=None, use_gpu=True):
    ...
    def run(self, save_dir='log', max_epoch=0, start_epoch=0, fixbase_epoch=0, open_layers=None,
        ...
        for epoch in range(start_epoch, max_epoch):
            self.train(epoch, max_epoch, trainloader,
                       fixbase_epoch, open_layers, print_freq)
    ...
    def _evaluate(self, epoch, dataset_name='', queryloader=None, galleryloader=None,
        ...
        for batch_idx, data in enumerate(queryloader):
            ...
            # TODO: ? cann't evaluate correctly when uncomment the following codes
        ...
        # save the neccessarily infos for testing
        save_for_test = {
            'distmat': distmat,
            'query': queryloader.dataset.query,
            'gallery': queryloader.dataset.gallery
        }
        with open('save_for_test.pkl', 'wb') as w_obj:
            pickle.dump(save_for_test, w_obj)
        ...
        for target in list(testloader.keys()):
            ...
            for batch_idx, data in enumerate(queryloader):
                ...
                for j in range(outputs.size(0)):
                    ...
                    grid_img = 255 * \
                        np.ones((height, 3*width+2*GRID_SPACING, 3),
                                dtype=np.uint8)
                    grid_img[:, :width, :] = img_np[:, :, ::-1]
                    grid_img[:, width+GRID_SPACING: 2 *
                             width+GRID_SPACING, :] = am
