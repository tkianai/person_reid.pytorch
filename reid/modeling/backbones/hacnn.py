...
class SpatialAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        # bilinear resizing
        x = F.upsample(x, (x.size(2)*2, x.size(3)*2),
...
class HACNN(nn.Module):
    ...
    def __init__(self, nchannels=[128, 256, 384], feat_dim=512, learn_region=True, **kwargs):
    ...
    def forward(self, x):
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x_local_i = F.avg_pool2d(x_local_i, x_local_i.size()[
                                         2:]).view(x_local_i.size(0), -1)
                x_local_list.append(x_local_i)
            x_local = torch.cat(x_local_list, 1)
            x_local = self.fc_local(x_local)

        return torch.cat([x_global, x_local], 1).unsqueeze(2).unsqueeze(3)


def hacnn(last_stride, **kwargs):
    model = HACNN(**kwargs)
    return model
