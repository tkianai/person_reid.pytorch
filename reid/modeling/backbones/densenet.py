...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, last_stride=2):
        ...
        self.add_module('pool', nn.AvgPool2d(kernel_size=2, stride=last_stride))
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, last_stride=2, **kwargs):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                if i == len(block_config) - 2:
                    trans = _Transition(
                        num_input_features=num_features, num_output_features=num_features // 2, last_stride=last_stride)
    ...
    def load_param(self, model_path):
        from .utils import init_pretrained_weights as load_pretrain
        load_pretrain(self, model_path)
...
def densenet121(last_stride, pretrained=True, **kwargs):
    model = DenseNet(
...
def densenet169(last_stride, pretrained=True, **kwargs):
    model = DenseNet(
        num_init_features=64,
        growth_rate=32,
...
def densenet201(last_stride, pretrained=True, **kwargs):
    model = DenseNet(
        num_init_features=64,
        growth_rate=32,
...
def densenet161(last_stride, pretrained=True, **kwargs):
    model = DenseNet(
