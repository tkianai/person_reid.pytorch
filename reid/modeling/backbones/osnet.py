...
__all__ = ['osnet_x1_0', 'osnet_x0_75',
           'osnet_x0_5', 'osnet_x0_25', 'osnet_ibn_x1_0']
...
class OSNet(nn.Module):
    ...
    def load_param(self, model_path):
        from .utils import init_pretrained_weights as load_train
        load_train(self, model_path)
...
def osnet_x0_75(pretrained=True, **kwargs):
    # medium size (width x0.75)
    model = OSNet(blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
                  channels=[48, 192, 288, 384], **kwargs)
    if pretrained:
        init_pretrained_weights(model, key='osnet_x0_75')
    return model


def osnet_x0_5(pretrained=True, **kwargs):
    # tiny size (width x0.5)
    model = OSNet(blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
                  channels=[32, 128, 192, 256], **kwargs)
    if pretrained:
        init_pretrained_weights(model, key='osnet_x0_5')
    return model


def osnet_x0_25(pretrained=True, **kwargs):
    # very tiny size (width x0.25)
    model = OSNet(blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
                  channels=[16, 64, 96, 128], **kwargs)
    if pretrained:
